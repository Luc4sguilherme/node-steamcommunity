...
SteamCommunity.prototype.getUserInventory = function (
	...
	async function get(inventory, currency, start, retries = 5) {
		const methods = [
			self.getUserInventoryContents.bind(
				self,
				start,
				userID,
				appID,
		...
		if (steamApisKey && steamApisKey.length) {
			methods.push(
				self.getUserInventorySteamApis.bind(
					self,
					start,
					steamApisKey,
					userID,
		...
		if (steamSupplyApiKey && steamSupplyApiKey.length) {
			methods.push(
				self.getUserInventorySteamSupply.bind(
					self,
					start,
					steamSupplyApiKey,
					userID,
		...
		if (steamApiKey && steamApiKey.length) {
			methods.push(
				self.getInventoryItemsWithDescriptions.bind(
					self,
					start,
					steamApiKey,
					userID,
		...
		try {
			const result = await methods.reduce(
				(promise, method) =>
					promise.catch((error) => {
			...
			body = result.body.response || result.body;

			if (
				typeof response.body != "object" ||
				response.body.fake_redirect
			) {
				if (retries > 0) {
			...
			if (!body || !body.success || !body.assets || !body.descriptions) {
				if (body) {
					callback(
						new Error(
							body.error || body.Error || "Malformed response"
						)
					);
				} else {
					callback(new Error("Malformed response"));
			...
			if (err.message == "HTTP error 403") {
				if (
					response.body &&
					response.body.includes("Invalid API key")
				) {
				...
				if (
					response.body &&
					response.body.includes("Inventory Private")
				) {
			...
			if (steamApiKey && response.statusCode == 403) {
				callback(new Error("Invalid API key"));
				return;
			}
	...
	function getDescription(descriptions, classID, instanceID) {
		...
		for (var i = 0; i < descriptions.length; i++) {
			quickDescriptionLookup[
				descriptions[i].classid +
					"_" +
					(descriptions[i].instanceid || "0")
			] = descriptions[i];
		}
 ...
 * @param {function} callback
 */
SteamCommunity.prototype.getInventoryItemsWithDescriptions = function (
	start,
	apiKey,
	userID,
	appID,
 ...
 * @param {function} callback
 */
SteamCommunity.prototype.getUserInventorySteamApis = function (
	start,
	apiKey,
	userID,
	appID,
 ...
 * @param {function} callback
 */
SteamCommunity.prototype.getUserInventorySteamSupply = function (
	start,
	apiKey,
	userID,
	appID,
